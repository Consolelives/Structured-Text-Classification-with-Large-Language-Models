        ...
        for idx, row in self.df.iterrows():
            ...
            for cat in categories:
                ...
                if pd.notna(subfolder):
                    ...
                    file_path = os.path.join(folder_path, f'{row["nos"]}.txt')
    ...
    def save_april(self):
        ...
        for idx, row in filtered_dfs.iterrows():
            ...
            with open(file_path, 'w', encoding='utf-8') as f:
                ...
                f.write(f"Named Entities:\n{row['named_entities'] if not (isinstance(row['named_entities'], (list, np.ndarray, pd.Series)) and all(pd.isna(row['named_entities']))) " 
                        f"and pd.notna(row['named_entities']) else ' '}\n\n")
                f.write(f"April Events:\n{row['april_events'] if (isinstance(row['april_events'], list) and len(row['april_events']) > 0) else (row['april_events'] if pd.notna(row['april_events']) else '')}\n")
